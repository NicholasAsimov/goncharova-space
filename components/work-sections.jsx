...
export function HeroArtCloud({ slugs = HERO_ART_SLUGS, className = "" }) {
  ...
  return (
    <div className={`relative h-[480px] w-full overflow-hidden rounded-[26px] border border-black/10 bg-white md:h-[620px] ${className}`}>
      ...
      {heroArt.map((project, index) => {
        ...
        return (
          <Link
            ...
            className={`absolute ${positions[index % positions.length]} art-float block overflow-hidden rounded-2xl border border-black/15 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.2)]`}
  ...
  return (
    <section id={id} className="border-t border-black/10">
      <div className="mx-auto w-full max-w-[1500px] px-4 py-16 md:px-8 md:py-24">
        <h2 className="text-balance text-[clamp(2.1rem,5.2vw,5rem)] font-black uppercase leading-[0.92] tracking-[-0.03em] text-black">
          {title}
        </h2>

        <div className="mt-7 overflow-hidden rounded-full border border-black/20 bg-white py-2">
          ...
          {projects.map((project, index) => {
            ...
            return (
              <Link
                key={project.slug}
                href={`/projects/${project.slug}`}
                className="group overflow-hidden rounded-2xl border border-black/15 bg-white"
              >
                <figure className="relative overflow-hidden">
                  ...
                  />
                  <div className="absolute left-2 top-2 rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-black" style={{ backgroundColor: accent }}>
                    {project.year}
                  </div>
                </figure>
